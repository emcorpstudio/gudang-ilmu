package emcorp.studio.sinergienerginegeri.data;
...
//import emcorp.studio.sinergienerginegeri.model.CardViewImg;
//import emcorp.studio.sinergienerginegeri.model.Image;
//import emcorp.studio.sinergienerginegeri.model.Inbox;
//import emcorp.studio.sinergienerginegeri.model.MusicAlbum;
//import emcorp.studio.sinergienerginegeri.model.MusicSong;
import emcorp.studio.sinergienerginegeri.model.Donation;
//import emcorp.studio.sinergienerginegeri.model.People;
//import emcorp.studio.sinergienerginegeri.model.ShopCategory;
//import emcorp.studio.sinergienerginegeri.model.ShopProduct;
//import emcorp.studio.sinergienerginegeri.model.Social;
//import emcorp.studio.sinergienerginegeri.utils.MaterialColor;
...
public class DataGenerator {
    ...
    public static List<String> getStringsMediumDonation(Context ctx) {
        List<String> items = new ArrayList<>();
        String name_arr[] = ctx.getResources().getStringArray(R.array.strings_medium_donation);
        for (String s : name_arr) items.add(s);
        Collections.shuffle(items);
        return items;
    }
    ...
    public static List<String> getFullDateDonation(Context ctx) {
        List<String> items = new ArrayList<>();
        String name_arr[] = ctx.getResources().getStringArray(R.array.full_date_donation);
        for (String s : name_arr) items.add(s);
        Collections.shuffle(items);
        return items;
    }
    ...
    public static List<Integer> getAllImagesDonation(Context ctx) {
        List<Integer> items = new ArrayList<>();
        TypedArray drw_arr = ctx.getResources().obtainTypedArray(R.array.all_images_donation);
        for (int i = 0; i < drw_arr.length(); i++) {
            items.add(drw_arr.getResourceId(i, -1));
        }
        Collections.shuffle(items);
     ...
     * @return list of object
     */
    public static List<Donation> getDonationData(Context ctx, int count) {

        List<Donation> items = new ArrayList<>();

        List<Integer> images = getAllImagesDonation(ctx);
        List<String> titles = getStringsMediumDonation(ctx);
        List<String> full_date = getFullDateDonation(ctx);
        String cat[] = ctx.getResources().getStringArray(R.array.news_category);

        for (int i = 0; i < count; i++) {
            Donation obj = new Donation();
            obj.judul = titles.get(i);
            obj.full_name = cat[i];
            obj.created_at = full_date.get(i);
            items.add(obj);
        }
        return items;
    }
